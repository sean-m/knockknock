...
namespace Gnu.Getopt
{
	...
	public class Getopt
	{
		...
		/// </summary>
		private ResourceManager resManager = knockknock.Properties.Resources.ResourceManager;
		...
		public int getopt () {	// not capitalized because of an compiler error
			...
			*/
			...
			if ((temp.Length > 1) && (temp [1] == ':')) {
				if ((temp.Length > 2) && (temp [2] == ':')) {				// This is an option that accepts and argument optionally
					if (this.nextchar.Length != 0) {
						this.optarg = this.nextchar;
						++this.optind;
					} else {
